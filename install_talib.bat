...
IF ERRORLEVEL 1 (ECHO [错误] pip 升级失败 & PAUSE & EXIT /B 1)
PAUSE
...
IF EXIST "%TA_LIB_DIR%\c\include\ta_lib.h" (
    ...
    IF ERRORLEVEL 1 (ECHO [错误] 下载失败 & PAUSE & EXIT /B 1)
...
IF ERRORLEVEL 1 (ECHO [错误] TA-Lib python 包安装失败 & PAUSE & EXIT /B 1)
PAUSE
...
IF ERRORLEVEL 1 (ECHO [失败] import talib 仍不可用，请查看上方报错 & PAUSE & EXIT /B 1)
PAUSE
