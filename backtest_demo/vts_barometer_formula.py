...
数据：SQLite（~/.vntrader/database.db）中 VIX / VXV(=VIX3M) / VVIX(=^VVIX) / SPY
VVIX 为真实抓取（2026-07-17 补）；^VIX9D 仍缺（文档标注可选增强）。共 15 个指标。
...
def build_barometer():
    ...
    v = vix.to_numpy(); x = vxv.to_numpy(); vv = vvix.to_numpy(); s = spy.to_numpy()
    n = len(v)
    dates = np.array(idx)
